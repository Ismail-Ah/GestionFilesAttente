    ...
    .catch(error => {
      if (error.response) {
        if (error.response.status === 422) {
          ...
          this.errorMessage = `Erreur ${error.response.status}: ${error.response.statusText}. Détails: ${error.response.data.message || 'Aucun détail fourni.'}`;
        }
      } else if (error.request) {
        this.errorMessage = 'La requête a été envoyée, mais aucune réponse n\'a été reçue.';
      } else {
        this.errorMessage = `Erreur lors de la configuration de la requête: ${error.message}`;
      }
    })
    .finally(() => {
